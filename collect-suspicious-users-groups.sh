...
LOG_PATH="/tmp/collect-suspicious-users.log"; LOG_MAX_KB=100; LOG_KEEP=5
...
while IFS=: read -r user x uid gid desc home shell; do
  ...
  [[ $pwinfo =~ \! ]] && locked=true
  if (( uid<1000 )); then continue; fi
  if $noexpiry || ! $locked; then
    sus_users+=("$(jq -n --arg u "$user" --arg uid "$uid" --arg home "$home" --arg shell "$shell" \
                   --arg noexp "$noexpiry" --arg locked "$locked" '{user:$u,uid:($uid|tonumber),home:$home,shell:$shell,password_never_expires:($noexp=="true"),account_locked:($locked=="true")}')")
  fi
done <<< "$passwd"
...
while read -r line; do
  ...
  flagsudo+=("$(jq -n --arg user "$user" --arg entry "$line" '{user:$user,sudo_entry:$entry}')")
done <<< "$sudoers"
